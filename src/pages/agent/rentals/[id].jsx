...
function EditListing() {
  ...
  const { isLoading, serverError, apiData } = useFetchData(`/api/rentals/${id}`);
  ...
  if (!isLoading && singleListing) {
    const { address, price, propertyType, bedrooms, bathrooms, keyFeatures, description } =
      singleListing.data[0];
      ...
      <Protected>
        <div className="dashboard-container center w-100">
          ...
          <div className="agent-summary text-center">
            <InlineLink to="/agent/rentals" text="Back to Rentals Dashboard" />
          </div>
        </div>
        <div className="w-100 two-column-grid">
          ...
          <div className="w-100">
            <ImageForm
              ...
              redirectUrl="/agent/rentals"
              apiEndpoint={`/api/rentals/${id}/image`}
            />
            {apiData?.data[0]?.images.length !== 0 ? (
              <DeleteImages images={apiData.data[0]?.images} type="rentals" />
            ) : (
              <></>
            )}
          </div>
